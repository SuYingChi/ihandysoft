...
public class BoostIconDrawable extends AnimatedIconDrawable implements RamUsageDisplayUpdater.RamUsageChangeListener {
    ...
    private void postResult() {
        LauncherTipManager.getInstance().showFinishBoostAlert(mContext, BoostSource.DESKTOP_DRAWABLE, mResultType, mPercentageBoosted, mAd);
    ...
    public void start() {
        ...
        if (!BoostTipUtils.mayShowChargingScreen(mResultType)
    ...
    private void drawProgressText(Canvas canvas, int percentage, @ColorInt int color, float scale, int alpha) {
        ...
        canvas.drawText(percentString,
                (float) Math.floor(mCenterX - 0.73f * mTempRect.width()),
