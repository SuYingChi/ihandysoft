...
public class LockHelper {
    ...
    public static boolean isInstantlyLockEnable(Context context) {
        ...
        try {
            Class cls = ReflectionHelper.getClass(clazzName);
            Class parType = Context.class;
            Object utilInstance = ReflectionHelper.getConstructor(cls, parType).newInstance(context.getApplicationContext());
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                Method method = ReflectionHelper.getDeclaredMethod(cls, methodName, int.class);
                return (boolean) method.invoke(utilInstance, Process.myUserHandle().hashCode());
            } else {
                Method method = ReflectionHelper.getDeclaredMethod(cls, methodName);
