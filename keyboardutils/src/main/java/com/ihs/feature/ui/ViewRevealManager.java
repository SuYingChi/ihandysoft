...
public class ViewRevealManager {
    ...
    protected ObjectAnimator createDynamicAnimator(RevealValues data) {
        ObjectAnimator animator =
                ObjectAnimator.ofFloat(data, REVEAL, data.startRadius, data.endRadius);
...
//        return revealData != null && revealData.applyTransformation(canvas, child);
        ...
        return transform(canvas, child, null, null);
    }

    public boolean transform(Canvas canvas, View child, Float cx, Float cy) {
        final RevealValues revealData = targets.get(child);
        if (cx == null || cy == null) {
            return revealData != null && revealData.applyTransformation(canvas, child);
        } else {
            return revealData != null && revealData.applyTransformation(canvas, child, cx, cy);
        }
    }
    ...
    public static final class RevealValues {
        ...
        boolean applyTransformation(Canvas canvas, View child) {
            return applyTransformation(canvas, child, null, null);
        }

        boolean applyTransformation(Canvas canvas, View child, Float cx, Float cy) {
