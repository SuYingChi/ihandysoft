...
public class PermissionAsyncTimer {
     ...
     * @return Timer 对象
     */
    public static PermissionAsyncTimer runAsync(Runnable runnable) {
        return schedule(0, 0, false, new Handler(), runnable);
    }

    private static PermissionAsyncTimer schedule(long delay, long period, boolean repeat, Handler handler, Runnable runnable) {
        PermissionAsyncTimer timer = new PermissionAsyncTimer();
                ...
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        if (!mCanceled && mRunnable != null) {
                            mRunnable.run();
                            if (!mRepeat) {
                                PermissionAsyncTimer.this.cancel();
                            }
                        }
                    }
                });
     ...
     * @return Timer对象
     */
    public static PermissionAsyncTimer runAsync(Runnable runnable, long delay) {
     ...
     * @return Timer对象
     */
    public static PermissionAsyncTimer schedule(long period, Runnable runnable, long delay) {
