...
public class FloatWindowControllerImpl {
    ...
    public void showChargingScreen(Bundle bundle) {
        ...
        if (!LockerUtils.hasPermission(android.Manifest.permission.SYSTEM_ALERT_WINDOW)
                || !PermissionImpl.canDrawOverlays()) {
            return;
        }
        ...
        if (!addedToWindowMgr) {
            ...
            container = (ViewGroup) LayoutInflater.from(context).inflate(R.layout.charging_window, null);
    ...
    private void doHideLockScreen(boolean dismissKeyguard) {
        if (addedToWindowMgr) {
            ...
            try {
                if (LockerUtils.hasPermission(Manifest.permission.SYSTEM_ALERT_WINDOW)) {
